...
class MonitorBase(object):
    '''Defines the base class for monitors to be used with ESDL
    defined systems.
    
    `MonitorBase` can also be used as a 'do-nothing' monitor.
    
    While `MonitorBase` does not make use of it, configuration
    dictionaries are supported. The default initialiser accepts
    a configuration which it will overlay over child ``syntax``
    ...
    def on_notify(self, sender, name, value):
        '''Called in response to notifications from other objects.
        For example, a mutation operation may call ``notify`` to
        report to the monitor how many individuals were mutated. The
        monitor receives this message through `on_notify` and either
        ignores it or retains the statistic.
    ...
    def _on_notify(self, sender, name, value):
        ...
        :Warn: Do not override this method to handle messages.
    ...
    def notify(self, sender, name, value):
        '''Sends a notification message to this monitor. This is used
        in contexts where a reference to the monitor is readily
        available. If the global ``notify`` function is available
        (for example, in selectors, generators or evaluators) it
    ...
    def on_pre_reset(self, sender):
        '''Called when the groups are reset, generally immediately
        after `on_run_start` is called.
        
        If this function or the system reset raises an exception,
        it will be passed to `on_exception`, `on_run_end` will be
        called and the run will be terminated.
    ...
    def on_post_reset(self, sender):
        '''Called immediately after the initialisation code specified
        in the system definition has executed.
    ...
    def on_pre_breed(self, sender):
        ...
        If this function or the system breed raises an exception,
        it will be passed to `on_exception`, `on_post_breed` will be
        called and if `should_terminate` returns ``False`` execution
    ...
    def on_run_start(self, sender):
        ...
        If this function raises an exception, it will be passed
        to `on_exception`, `on_run_end` will be called and the run
        will be terminated.
    ...
    def on_run_end(self, sender):
        '''Called at the end of a run, regardless of the reason for ending.
    ...
    def should_terminate(self, sender):     #pylint: disable=R0201,W0613
        '''Called after each experiment step to determine whether to terminate.
        
        :Parameters:
          sender : `System`
            The system instance reporting to this monitor.
        
        :Returns: ``True`` if the run should terminate immediately; otherwise,
                  ``False``.
        '''
        return True
