...
class TourIndividual(IntegerIndividual):
    '''An `Individual` with tour genomes. These individuals are used
    with `TourSpecies` and `tsp.Landscape` to solve the Travelling
    Salesman Problem.
    
    The genome is a list of nodes in the order that they are visited,
    represented as integer values. The phenome is a list of pairs
    (tuples) of nodes, each representing a link from the first node to
    the second node.
    '''
    ...
    def phenome(self):
        ...
        of pairs (tuples) of nodes, each representing a link from the
        first node to the second node.
        '''
        p = [ ]
        current = self.genome[0]
        for g in islice(self.genome, 1, None):
            p.append((current, g))
            current = g
    ...
    def phenome_string(self):
        ...
        individual.
        '''
        i = self.genome.index(0)
        return ' ->'.join('%3d' % (city + 1)
            for city in chain(islice(self.genome, i, None),
                              islice(self.genome, 0, i)))
...
class Landscape(landscape.Landscape):
    ...
    def _eval(self, indiv):
        ...
        try:
            return sum(cost_map[p] for p in indiv.phenome)
        except KeyError:
